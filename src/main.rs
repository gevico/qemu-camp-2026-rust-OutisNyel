...
async fn main() {
    ...
    match command {
        ...
        Subcommands::CicvVerify(_subargs) => {
            ...
            let mut exercise_check_list = ExerciseCheckList {
                exercises: vec![],
                user_name: None,
            ...
            for exercise in exercises {
                ...
                match run(&exercise, true) {
                    // match verify(vec![&inner_exercise], (0, 1), true, true) {
                    Ok(_) => {
                        exercise_check_list.statistics.total_succeeds += 1;
                        println!("{}执行成功", exercise.name);
                        println!("总的题目数: {}", alls);
                        println!(
                            "当前做正确的题目数: {}",
                            exercise_check_list.statistics.total_succeeds
                        );
                        println!("当前修改试卷耗时: {} s", exercise_start.elapsed().as_secs());
                        exercise_check_list.exercises.push(ExerciseResult {
                            name: exercise.name,
                            result: true,
                        });
                    }
                    Err(_) => {
                        ...
                        exercise_check_list.statistics.total_failures += 1;
                    }
                }
            }
            let total_time = now_start.elapsed().as_secs();
            println!("===============================试卷批改完成,总耗时: {} s; ==================================", total_time);
            exercise_check_list.statistics.total_time = total_time as u32;
            let serialized = serde_json::to_string_pretty(&exercise_check_list).unwrap();
